...
Our dataset follows S&P500 companies with 16 different variables quantifying their performance on the New York Stock Exchange as of 2017-12-31 (obtained via the Quandl API). Each row represents a company's, with column metrics ranging from Gross Profit and Return on Equity (ROE), to Assets and Liabilities. In this report, we want to answer two questions. Firstly, we want to measure the predictive ability of a given company's key financial metrics regarding the stock price performance between from 2017-12-31 to 2018-12-31. Secondly, we want to see whether we can cluster companies using the given financial variables, and use these clusters in our stock performance prediction. In answering these questions, we will use four statistical techniques:

Firstly, we will conduct Principal Component Analysis (PCA). In this analysis, we will try to see which performance metrics tend to vary greatly between stocks. Our PCA will yield a few uncorrelated principal components in the directions of greatest variance. This will allow us to see which performance metrics tend to correlate with one another (their weights within principal components), and reduce the overall dimensionality and effectively eliminate any correlation between variables to avoid any bias that would be introduced by multicollinearity.

Secondly, we will use Discriminant Analysis (DA). This method will allow us to see which principal components (and by extension the performance metrics contained in their loading coefficients) are most important for discriminanting between companies that did or did not outperform the SP500 during that same timeframe.


Thirdly, we will use Cluster Analysis. By using this method, we will investigate if there are  particular performance metrics that can be used to decern which Sector a stock belongs to. We will then interpret the results of our cluster analysis and talk about the predictive ability of each clusters for stock performance.

Lastly, to supplement our Cluster Analysis, we will use MANOVA. In this section, we will try to see the effects of sectors on the financial ratios we used in predicting stock price performance. 

Before starting our analysis, we want to tell more about the data we are using. Our dataset has both categorical and continuous variables. Categorical Variables (3) include 'Ticker Symbol,' 'S&P500 Sector,' and 'Returns' (how much did the stock price percent change with respect to the S&P500 percent change within the same timeframe?). Continuous Variables (13) include mainly key financial ratios. The reason we preferred using ratios is that we wanted to have variables that are comparable across companies. Eventually, continuous variables we used were: 'Cash&Equivalents/Liability', 'Equity/Assets', 'EBIT/EV', 'EV/EBITDA', 'EBITDA Margin', 'Gross Margin', 'Net Margin', 'ROA (Return on Asset)', 'ROE(Return on Equity)', 'ROIC (Return on Invected Capital)', 'ROS (Return on Sales)', EV (Entreprise Value) and 'YoY change in Stock Price'.

For reference, we also provide a short description of the main variables we are using: 
...
After log transforming cashneq_liabilities and removing outliers, the variables all appear to be approximately univariate Normal (save for a few pesky outliers).
...
We observe that we have some highly correlated variables. PCA will work well for our data.
...
In comparing the PCA values for the mediocre, high, and low return stocks for the first three dimensions, we notice that the distribution of PCA values by returns appears largely symmetric. It should be noted that there is some noticable skewness for the mediocre returns grouping in the second principle component and in the high returns grouping for the third principle component. With this symmetry, we note that the our boxplots of the principle components suggest an approximately normal distribution for each group.

```{r}
plot(PCA_Data[,-c(1,2)], col = 2*(as.numeric(PCA_Data$Returns)), pch = as.numeric(PCA_Data$Returns)+15, cex=1.2)
```

An inspection of the principal components' projected scatters appears promising. All three groups (each has a different shape and color) seem to have a similar random scatter and "covariance footprint." In other words, it is a strong possibility that the data is multivariate Normal within groups, and the group covariance matricies are not significantly different from one another.
...
For completeness, we also run quadratic discrimant analysis, even though as we noted above, our covariance matrices are similar between groups as determined by the Box-M test. In the confusion matrix above computed for the QDA with Cross-Validation classification, we find that the accuracy rate is lower at 41.2%. Again, this figure is marginally better than guessing, and so we again find that the accuracy is lacking. 

```{r}
partimat(Returns~PC1+PC2+PC3, data=PCA_Data, method="lda")
```

We plot the classification via LDA results in two dimensional space to better visualise the accuracy of the classification method. In plotting the classifications on two dimensions between PC1, PC2, and PC3, we find that in each case, the error rate is marginally better than guessing as the error rates are between 0.549 and 0.585.
...
We again plot the LDA classification results, through ggplot for better aesthetics, that again illustrates the stark contrast between predicted and actual returns. The black lines delineate the decision rules to classify the data points for LDA with respect to the linear discriminant dimensions. In observing the true returns figure, we recognise that LDA does not discriminate high, low, and mediocre stock returns accurately.
...
```
Looking at the posterior probability coloured by accuracy, we note that there appears to be no discernable relationship between higher posterior probability and accuracy. If DA were effective, we would expect that as the posterior probability-- which we can understand here as 'confidence' in the discrimination-- increases, the accuracy would similarly increase. As the plot above shows, however, that relation doesn't hold.
...
\\ NEED MORE ANALYSIS HERE ***[DAVID WILL WRITE THIS PART!!!]***
...
In this section, we will try to answer the following question: if we choose to cut our data into 10 clusters, will the companies then be grouped roughly into their respective sectors?"
...
This graph gives us a good overview of the data clusters, but we want to zoom in and see what each cluster holds. 
...
We decided to use the distance-linkage combination which maximized the mean membership rate across all clusters, while also giving us the most distinct clusters. This turns out to be using *"Euclidean"* distance and *"complete"* linkages, which makes sense: "complete" linkages use the maximum distance between clusters and tend to make trees with distinct clusters, while also being less sensitive to noisy data--which is good for us, given that we have 335 stocks!--and Euclidean distance is a common method to use, especially as we have no reason to suggest that Manhattan would be better for the similarity or dissimilarity between stocks. This gives us an average membership rate of .36418. Listing our clusters by the industry most represented in each with the proportion of represented gives us the following results, as shown in this table: 

```{r}
industries <- c("Industrials", "Discretionary", "Healthcare", "Healthcare", "Utilities", "Financials", "Discretionary", "Financials", "Real Estate", "Financials" )
rates <- c("0.2759", "0.2237", "0.3478", "0.3846", "0.5", "0.7667", "0.439", "0.2069", "0.8667", "0.1667")

cluster_table <- data.frame(rates, industries)
cluster_table
```
