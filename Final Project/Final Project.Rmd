...
missing_prices <- c(NA, NA, NA, NA, NA, NA, 43.04, 29.78, NA, 14.66, NA, 47.78, 44.74, NA, 71.34, NA)
...
remove <- c(as.vector(which(working_data$sector %in% c("Telecommunication Services"))), 125)
...
## NEED TO ASK JDRS ABOUT CISCOREPLOT ERRORS
```{r}
...
```{r}
plot(PCA_Data[,-c(1,2)], col = 2*(as.numeric(working_data$returns)), pch = as.numeric(working_data$returns)+15, cex=1.2)
```
...
data.LDA <- data_transformed # the data 
group.LDA <- as.factor(working_data$returns) # the classes 

# starting with roic because it has the highest pearson correlation with percent change

stepwise.lda <- stepclass(data.LDA, group.LDA, "lda", start.vars = "roic")
stepwise.lda
plot(stepwise.lda)
...
confusion <- table(group.LDA, predict(LDA)$class)
...
confusion <- table(group.LDA, cv.LDA$class)
...
```{r}
scores <- data.frame(Predicted_Returns=predict(LDA)$class, predict(LDA)$x, True_Returns=group.LDA)
boundary_function <- lda(scores[,2:3], scores[,1])

x <- seq(min(scores[,2]), max(scores[,2]), length.out=nrow(scores))
y <- seq(min(scores[,3]), max(scores[,3]), length.out=nrow(scores))


discriminant_value_grid <- matrix(c(rep(x,length(y)), rep(y, rep(length(x), length(y)))),,2)

boundary_1 <- predict(boundary_function, discriminant_value_grid)$post[, c("High","Mediocre")] %*% c(1,1)
boundary_2 <- predict(boundary_function, discriminant_value_grid)$post[, c("Mediocre", "Low")] %*% c(1,1)

boundaries <- data.frame(x=rep(x, length(y)), y=rep(y, each=length(x)), z1=as.vector(boundary_1), z2=as.vector(boundary_2))


ggplot(scores, aes(x=LD1, y=LD2)) + 
    geom_point(size = 3, aes(pch=Predicted_Returns, col=Predicted_Returns)) + 
    geom_contour(data=boundaries, aes(x=x, y=y, z=z1), breaks=c(0, 0.5), size=1.5, color="black") + 
    geom_contour(data=boundaries, aes(x=x, y=y, z=z2), breaks=c(0, 0.5), size=1.5, color="black") +
    ggtitle("Predicted Returns") + theme(plot.title = element_text(hjust=0.5, size=20), legend.title=element_blank())

ggplot(scores, aes(x=LD1, y=LD2)) + 
    geom_point(size = 3, aes(pch=True_Returns, col=True_Returns)) + 
    geom_contour(data=boundaries, aes(x=x, y=y, z=z1), breaks=c(0, 0.5), size=1.5, color="black") + 
    geom_contour(data=boundaries, aes(x=x, y=y, z=z2), breaks=c(0, 0.5), size=1.5, color="black") +
    ggtitle("True Returns") + theme(plot.title=element_text(hjust=0.5, size=20), legend.title=element_blank())
```

```{r}
posterior.LDA <- apply(predict(LDA)$posterior, 1, max)

correct_or_not <- as.factor(predict(LDA)$class == group.LDA)
...
for (j in 1:20) {
  for (i in 1:45) {
    ...
    knn_prediction <- as.vector(knn(train = train_data, test = test_point, cl = group.LDA[-i], k = j))
    truth <- as.vector(group.LDA[i])
...
#   percent_correct[i] <- sum(diag(prop.table(misclass.test)))/30
...
# misclass #out of 900
# 
# sum(diag(prop.table(misclass)))/900
```
