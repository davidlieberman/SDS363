...
--Third Principal Component Major Contributors: equality_assets (0.434), ebit_ev (-0.541), ev_ebitda (0.574)
...
```{r}
LDA <- lda(Returns ~ PC1 + PC2 + PC3, PCA_Data) 
scores <- data.frame(Predicted_Returns=predict(LDA)$class, 
                     predict(LDA)$x, 
                     True_Returns=factor(PCA_Data$Returns))

PC1 <- seq(min(PCA_Data$PC1), max(PCA_Data$PC1), length.out=10)
PC2 <- seq(min(PCA_Data$PC2), max(PCA_Data$PC2), length.out=10)
PC3 <- seq(min(PCA_Data$PC3), max(PCA_Data$PC3), length.out=10)

contour_grid <- expand.grid(PC1=PC1, PC2=PC2, PC3=PC3)
contour_prediction <- predict(LDA, newdata=contour_grid)
contour_data <- data.frame(contour_prediction$x, z=as.numeric(contour_prediction$class))

ggplot(data = contour_data, aes(x = LD1, y = LD2)) +
  geom_point(size = 3, aes(pch=factor(z), col=factor(z)))

# ggplot(contour_data, aes(x = LD1, y = LD2)) +
#   geom_point(size=3, aes(pch=c(1,2,3), col=c(1,2,3))) +
#   stat_contour(aes(x = LD1, y = LD2, z = z), data = contour_data, size=1, color="black")
```


## NEED TO DISCUSS WITH JDRS
```{r}
...
\\ NEED MORE ANALYSIS HERE (I'll write this part)
