...
**We utilise the Euclidian distance method to compute distances and intiate clustering. Euclidean distance is particularly meaningful as our subsetted data set only includes continuous variables. We utilise Ward's method for hierarchical clustering. There are two reasons for this. Firstly, there may be a few outliers, and simple linkage is sensitive to outliers. Secondly, as explained in the lecture "Several studies of clustering suggest that Ward’s method or Average Linkage work the ‘best’" (pg. 324, Cluster Analysis Lecture). We opt for 7 clusters to begin when we plot the dendogram as we hypothesise that the stocks could potentially be divided along industry lines as mentioned prevoiusly. Our observations of the stocks suggest that there are 7 relevant industries. We included these industries and stock breakdown in the end of this analysis, under Question 5. When we perform distance cluster analysis with 7 clusters, the result seems to match our hypothesis--that is breakdown of companies supports a similar narrative.**
...
We calculate the internal sum of squares distances through 250 random partitions with the same number of clusters, each of the same sample size as their respective k-means partition counterparts.
...
This analysis focuses on the difference between the average of the red lines and blue lines. In choosing the number of clusters, we observed the point at which adding more clusters did not really bring further improvement. We see that after 6 clusters, we do not observe much change in the difference between the averages. This matches the result we obtained in the previous section in our analysis that included 250 simulated data sets.
