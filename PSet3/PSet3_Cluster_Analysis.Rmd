...
plot(data_clust_2,  main = "Plain dendrogram", hang = -1, cex = .5,
     xlab = "", ylab = "", sub = "", axes = FALSE)
```
**Model 2 used a "single" agglomeration method, which ought to produce more distinct groups. In this case, however, we obtained one large cluster that includes most of the stocks, and a few scattered ones toward the left-hand side of the dendogram. This does not appear to be particularly useful for cluster analysis, since there is so much intra-group variation in our very large cluster.**
...
```
**Since Manhattan distance calculation could work well for high-dimensional datasets, and also places less weight upon outliers, we also decided to try a model using this distancing method and "complete" agglomeration.**
...
# Make dendrogram
plot(data_clust_4, xlab="",ylab="Distance",main="Clustering for Stocks", cex = .5)
