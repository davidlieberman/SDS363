...
**We utilise the Euclidian distance method to compute distances and intiate clustering. Although we are cognisant of the succeptibility of Euclidean distance to outliers, we note that our data does not appear to have significant outliers, as revealed after plotting boxplots of our variables. We utilise Ward's method for hierarchical clustering as "Several studies of clustering suggest that Ward’s method or Average Linkage work the ‘best’" (pg. 324, Cluster Analysis Lecture). We opt for 7 clusters to begin when we plot the dendogram as we hypothesise that the stocks could potentially be divided along industry lines as mentioned prevoiusly. Our observations of the stocks suggest that there are 7 relevant industries.**
...
We calculate internal sum of squares distances through 250 random partitions into an equivalent number of clusters with the same sample sizes as we actually created with our actual k-means partition.

We look at the distance between our internal sum of squares blue line and red line, and ask at what point does the difference stops decreasing (improving). That point becomes our cluster number, and in this particular case, it is 6 clusters.
...
This is looking at the difference between the average of the red lines and blue lines. At what point, do adding more clusters do not really add much. We see that after 6 clusters, we do not observe further improvement. This matches the result we obtained in the first part of Question 4, in our analysis on 250 simulations.
...
In Principal Component space, we see that the two major principal components account for ....% of the variability.
```{r}
